...
export const Slaider = () => {
  ...
  const scrollLeft = () => {
    setCurrentIndex((prev) =>
      prev === 0
        ? Math.floor((allCards.length - 1) / cardsPerPage) * cardsPerPage
        : prev - cardsPerPage
    );
  };
  ...
  const visibleCards = [...allCards, ...allCards].slice(
    currentIndex,
  ...
  return (
    <div className={sl.petCarouselContainer}>
      ...
      <div className={sl.carouselWrapper}>
        <button
          className={`${sl.scrollButton} ${sl.left}`}
          onClick={scrollLeft}
        >
          ←
        </button>
        ...
        <button
          className={`${sl.scrollButton} ${sl.right}`}
          onClick={scrollRight}
        >
          →
        </button>
